 ...
 *   3. **导入即丢精度** —— 金蝶原始金额是 4 位定点（见下），
 *      本项目只保留 2 位，导入时 0.0020 这类精度由**我们自己**舍掉。
 *   4. 多币种会放大误差 —— 外币 × 汇率 × 汇率 的浮点累积，
 *      而金蝶特意保留 4 位正是为这个场景。
 *
 * 改造方向（若做，应一次做完，避免「半浮点半定点」的中间态）：
 *   · 金额字段 → 定点整数，单位 **×10000（万分之一元）**，与金蝶 CURRENCY 对齐
 *     （不用 ×100：那会在导入金蝶 4 位数据时自己引入舍入误差）
 *   · **只需改金额字段** —— 汇率/单价/数量保持浮点即可，金蝶也是如此
 ...
 *   等于在没有安全网的情况下重写核心计算。
 *
 * 金蝶侧的事实依据见 tools/read_ais.js 头部（可复现：跑该脚本读 .ais 表结构）。
 */
