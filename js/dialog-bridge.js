...
// HTML 浮层 kd-prompt-*（此类 Web 应用本就是网页风格，三平台体验一致且可靠）。
...
// 使用独有 class 名（kd-prompt-*）并关键样式内联，避免与页面既有 .modal/.modal-overlay 样式冲突。
function promptModal(message, def, opts) {
  return new Promise(function (resolve) {
    ...
    overlay.className = 'kd-prompt-overlay';
    ...
    overlay.innerHTML =
      '<div class="kd-prompt-modal" style="background:#fff;border-radius:10px;max-width:420px;width:90%;box-shadow:0 10px 40px rgba(0,0,0,.25);overflow:hidden;">' +
        '<div class="kd-prompt-hd" style="padding:14px 18px;font-weight:600;border-bottom:1px solid #eee;font-size:15px;">' + (opts && opts.title ? opts.title : '输入') + '</div>' +
        '<div class="kd-prompt-bd" style="padding:18px;">' +
          (safeMsg ? '<p style="margin:0 0 10px;color:#444;font-size:14px;">' + safeMsg + '</p>' : '') +
          '<input class="kd-prompt-input" type="text" value="' + safeDef + '" style="width:100%;box-sizing:border-box;padding:8px 10px;border:1px solid #ccd;border-radius:6px;font-size:14px;outline:none;">' +
        '</div>' +
        '<div class="kd-prompt-ft" style="padding:12px 18px;display:flex;justify-content:flex-end;gap:10px;border-top:1px solid #eee;">' +
          '<button class="kd-prompt-cancel" style="padding:7px 16px;border:1px solid #ccd;background:#fff;border-radius:6px;cursor:pointer;font-size:14px;">取消</button>' +
          '<button class="kd-prompt-ok" style="padding:7px 16px;border:none;background:#2f7d3a;color:#fff;border-radius:6px;cursor:pointer;font-size:14px;">确定</button>' +
        '</div>' +
      '</div>';
    document.body.appendChild(overlay);
    var input = overlay.querySelector('.kd-prompt-input');
    input.focus();
    ...
    overlay.querySelector('.kd-prompt-ok').onclick = function () { close(input.value.trim()); };
    overlay.querySelector('.kd-prompt-cancel').onclick = function () { close(null); };
...
// 因此本桥接一律用自建 HTML 浮层 kd-prompt-* 获取文本输入，不依赖任何系统原生 prompt，
