...
(function (global) {
  ...
  function convert(buffer, fileName) {
    ...
    vchRows.forEach(function (r) {
      ...
      //   代价是「本期发生额」按借贷双方合计列示（借 12,462.60 / 贷 12,462.60），
      //   而金蝶与准则主流口径的「借方发生额」是净额（借 10,737.75 / 贷 10,737.75）。
      // 【为什么可以原样保留】实测（构造含红字的账套）证明：保留负数时
      //   总账余额、试算平衡、会计恒等式、利润表、未结转损益全部正确；
      //   且「红字抵减发生额」正是《会计基础工作规范》第五十一条「红字冲账」的口径。
      //   注意：d/c 仍为下面 v.entries.push 使用的最终金额，故此处不做任何改写。
      v.entries.push({
        code: code,
    ...
    var ledger = {
      ...
      meta: {
        ...
        period: minPeriod,
        // 红字口径标记：'native' = 红字（负数）原样保留，与金蝶/准则口径一致。
        // 【为什么需要这个标记】旧版导入会把「借 -X」改写成「贷 +X」，导致「本期发生额」
        //   按借贷双方合计列示、与金蝶对不上；而那个改写不可逆。
        //   旧账套没有本字段 → 科目余额表据此提示口径差异（见 TrialBalance.js）。
        //   新版导入写入 'native' → 不再提示（口径已一致）。
        redStyle: 'native'
      }
    };
