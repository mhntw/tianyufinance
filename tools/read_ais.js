 ...
 *
 * ============================================================
 * 【附：金蝶 KIS 的金额精度设计 —— 实测结论，2026-09-19】
 *
 * 依据是 .ais **表结构里的字段类型定义**（用 reader.getTable(x).getColumns() 读取），
 * 不是 getData() 读出来的值。这个区分很重要，见下方「注意」。
 *
 * 实测（添钰来客_2026年_金蝶KIS格式.ais）：
 *   GLVch（凭证分录）
 *     FDebit / FCredit / FFcyAmt        类型 = CURRENCY   size = 8
 *     FExchRate / FQty / FPrice         类型 = DOUBLE     size = 8
 *   GLBal（科目余额）
 *     FDebit / FCredit / FBegBal / FEndBal / FYtdDebit / FYtdCredit
 *                                       类型 = CURRENCY   size = 8
 *     FAcctID = TEXT(100) / FCyID = TEXT(6)
 *
 * 结论：
 *   ① **金额用定点**：CURRENCY 即「64 位整数 + 隐含 4 位小数」，等价于 ×10000 的整数。
 *      绝不会出现 1560712.9919999999 这类浮点残留 —— 从存储层杜绝误差。
 *   ② **汇率 / 单价 / 数量用浮点（DOUBLE）**：它们是计算中间量（数量 × 单价 = 金额），
 *      误差只存在于中间步骤，最终一次性落到定点金额上。故金蝶**没有**给汇率做定点。
 *   ③ **精度为 4 位而非 2 位**：为多币种换算留余量（有独立的外币金额字段 FFcyAmt）。
 *
 * ⚠️ 注意（我初次分析时踩过的坑，勿重蹈）：
 *    用 mdb-reader 的 getData() 读金额，得到的是**字符串** "5421.0000" ——
 *    那只是**序列化的表现**，不是金蝶的存储类型。若据此推断「金蝶用字符串存金额」就错了。
 *    判断存储方式的唯一依据是 **getColumns() 返回的类型定义**。
 *
 * 对 ty 的启示：
 *   将来若把金额改为定点整数，应对齐 ×10000（与金蝶一致，可无损接收其 4 位数据）；
 *   且**只需改金额字段**，汇率/单价/数量可保持浮点 —— 金蝶也是如此。
 * ============================================================
 */
'use strict';
